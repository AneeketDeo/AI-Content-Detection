...
class TextAnalysisModule:
    ...
    def __init__(self, use_stylometric=True, use_perplexity=True, use_transformer=True):
        """
        Initialize the text analyzer with specified analyzers.
        
        Args:
            use_stylometric: Whether to use stylometric analysis
            use_perplexity: Whether to use perplexity analysis
            use_transformer: Whether to use transformer-based analysis
        """
        self.analyzers = {}
        
        # Initialize stylometric analyzer
        if use_stylometric:
            try:
                logger.info("Initializing stylometric analyzer")
                self.analyzers['stylometric'] = StylometricAnalyzer()
            except Exception as e:
                logger.error(f"Failed to initialize stylometric analyzer: {e}")
                self.analyzers['stylometric'] = None
        
        # Initialize perplexity analyzer
        if use_perplexity:
            try:
                logger.info("Initializing perplexity analyzer with model: gpt2")
                model_path = './models/gpt2'
                if os.path.exists(model_path):
                    self.analyzers['perplexity'] = PerplexityAnalyzer(model_path)
                else:
                    self.analyzers['perplexity'] = PerplexityAnalyzer()
            except Exception as e:
        ...
        if use_transformer:
            try:
                logger.info("Initializing transformer classifier with model: roberta-base-openai-detector")
                model_path = './models/roberta-base-openai-detector'
                if os.path.exists(model_path):
                    self.analyzers['transformer'] = TransformerClassifier(model_path)
                else:
                    self.analyzers['transformer'] = TransformerClassifier()
            except Exception as e:
